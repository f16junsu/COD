...
module Instruction_Cache(
    ...
    always @(posedge clk) begin
            if (cache_table[idx][`TAG_SIZE + `LINE_SIZE]) begin
                if (cache_table[idx][`TAG_SIZE-1 + `LINE_SIZE:`LINE_SIZE] == address[`WORD_SIZE-1:4]) begin
                    if (isStall) i_hit_counter <= i_hit_counter - 1; // preventing overlapping
                    end
                end
        ready <= 0;
    end
